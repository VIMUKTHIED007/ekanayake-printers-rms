...
namespace Ekanayake_Printers_RMS.Main_User_Controls
{
    public partial class UserAccount_Panel_Form : UserControl
    {
        ...
        {
            con = new SqlConnection("Data Source=THONKPAD;Initial Catalog=EKANAYAKE_PRINTERS_001;Integrated Security=True");
            string userID, userType, fullName ,userName, email,nic,empID,userAddress,pwd;
            int tel, positionID;
            DateTime Dob;
            ...
            {
                con.Open();
                cmd = new SqlCommand("INSERT INTO userTable (userID, fullName,NIC,email,userAddress ,tel, dob) VALUES (@a, @b, @c, @d, @e, @f,@g)", con);
                ...
                cmd.Parameters.AddWithValue("@e", userAddress);
                cmd.Parameters.AddWithValue("@f", tel);
                cmd.Parameters.AddWithValue("@g", Dob);

                if (positionID != 0 || empID!="")
                {
                    SqlCommand cmd2 = new SqlCommand("INSERT INTO employeetable (employeeID,userID,employeeType) VALUES (@h,@i,@j)", con);
                ...
                {
                    SqlCommand cmd3 = new SqlCommand("INSERT INTO userTable (userID,userName,pwd,occupationINDEX) VALUES (@k,@l,@m,@n)", con);
        ...
        private void btncusSearch_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string userid;
                con.Open();
                string query = "SELECT userID,fullName FROM userTable WHERE uNIC = @nic AND tel = @tel";
