...
namespace Ekanayake_Printers_RMS.Sub_UserControl
{
    public partial class Profile_User_Control : UserControl
    {
        ...
        private void GetOpenSession()
        {
            try
            {
                string connectionString = @"Data Source=LAPTOP-2956U5GC\SQLEXPRESS;Initial Catalog=EKANAYAKE_PRINTERS_001;Integrated Security=True";
        ...
        private void SidebarDetails()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    string queryGetUserDetails = "SELECT userID, userAddress, email, tel FROM userTable WHERE userID = @userID";
                    using (SqlCommand getUserData = new SqlCommand(queryGetUserDetails, conn))
                    {
                        ...
                        using (SqlDataReader dr4 = getUserData.ExecuteReader())
                        {
                            if (dr4.Read())
                            {
                                ...
                                label4.Text = dr4.IsDBNull(1) ? "NULL" : dr4.GetString(1); // userAddress
                    ...
                    string queryGetEmployeeDetails = "SELECT employeeType, department FROM employeeTable WHERE userID = @user_ID";
                    using (SqlCommand getEmployeeData = new SqlCommand(queryGetEmployeeDetails, conn))
                    {
                        ...
                        using (SqlDataReader dr5 = getEmployeeData.ExecuteReader())
                        {
                            if (dr5.Read())
                            {
                                label2.Text = dr5.IsDBNull(0) ? "NULL" : dr5.GetString(0); // employeeType
                                label8.Text = dr5.IsDBNull(1) ? "NULL" : dr5.GetString(1); // department
                            }
                            else
                            {
